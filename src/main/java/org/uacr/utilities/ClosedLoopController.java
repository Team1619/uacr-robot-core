...
public class ClosedLoopController {
    ...
    public double getWithPID(double measuredValue) {
        ...
        if (fCurrentProfile.hasForceCompensation) {
            return Math.signum(output) * Math.min(Math.abs(output), fCurrentProfile.maxOutput) + (fCurrentProfile.kForceCompensation * Math.sin(measuredValue));
        } else {
            return Math.signum(output) * Math.min(Math.abs(output), fCurrentProfile.maxOutput);
        }
    }
